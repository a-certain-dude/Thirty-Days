...
fun DayTitle(dayText: Int, modifier: Modifier = Modifier) {
    Text(text = stringResource(dayText), style = MaterialTheme.typography.displayLarge)
}

@Composable
fun TodoTittle(todoText: Int, modifier: Modifier = Modifier) {
    Text(text = stringResource(todoText), style = MaterialTheme.typography.displayMedium, modifier = modifier)
}

@Composable
fun ImageOfTheDay(dayImage : Int,modifier: Modifier = Modifier){
    Image(painter = painterResource(dayImage), contentDescription = stringResource(R.string.thirty_days_tips),  modifier = modifier)
}

@Composable
fun TextActivity(textActivity: Int,modifier: Modifier = Modifier){
    Text(text = stringResource(textActivity), style = MaterialTheme.typography.displayMedium,modifier = modifier)
}
